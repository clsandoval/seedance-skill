...
Load `references/quick-ref.md` for the checklist, `references/reference-workflow.md` for multimodal references, `references/i2v-guide.md` for image-to-video, `references/first-last-frame-guide.md` for first/last-frame work, `references/examples-by-mode.md` when examples are useful, `references/shot-list-continuity.md` for multi-shot professional plans, `references/multishot-grammar.md` for shot-label grammar, the shots-times-seconds budget, and cut placement inside one generation, and `references/multilingual-community-examples.md` for Chinese/Russian/Japanese/Korean/Spanish or mixed-language prompts.
