...
> **Execution backend is Replicate.** To actually run a generation, use `references/replicate-api.md` against the main `bytedance/seedance-2.0` model (never `-fast`). The surfaces below are background for capability cross-checks and pricing context, not the execution path.
...
Always load `references/api-status.md` for current API and platform claims. Load `references/model-name-map.md` when a user says Pro, Fast, V2, or a wrapper model ID. Do not rely on old release-status memory.
Load `references/api-workflow.md` for implementation planning, task lifecycle, Runway/Volcengine field differences, pricing caveats, upload handling, and production readiness.
Load `references/pro-filmmaking-standards.md` for professional film, commercial, agency, localization, post, and delivery workflows. Load `references/delivery-qc.md` before saying an asset is delivery-ready.
